...
class Evaluator(object):
    ...
    def eval_dataloader(self, model, dataloader, epoch=0):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, total=len(dataloader), desc="Evaluating epoch {}".format(epoch)):
                ...
                tot += B
                pred, cnt = model.forward_eval(batch)
                all_cnt += cnt
                preds.append(pred)
                gts.append(torch.stack([batch["start_frac"], batch["end_frac"]], dim=1))
        self.in_moment = all_cnt / tot
        print("In moment:", self.in_moment)
        loss /= len(dataloader)
    ...
    def report_in_moment(self):
        return "In moment: {}".format(self.in_moment)
