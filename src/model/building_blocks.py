...
class QueryGRUEncoder(nn.Module):
    def __init__(self, in_dim, dim, n_layers, dropout):
        ...
        self.positional_embedding = PositionalEmbedding(64, dim * 2) # use 2 * dim because bi-gru
...
class VideoSelfAttentionEncoder(nn.Module):
    ...
    def forward(self, video, video_mask):
        ...
        """
        temp, embeddings = self.video_embedding(video)
        for layer in self.layers:
            temp = layer(
                x=temp,
                mask=video_mask
            )
        return temp, embeddings


class MultiLevelEnhancement(nn.Module):
    def __init__(self, alpha, dim):
        super(MultiLevelEnhancement, self).__init__()
        self.alpha = alpha
        self.gate_fc = nn.Linear(2 * dim, dim)

    def forward(self, modal1_feature, modal2_feature, attn_weight):
        """
        Args:
            modal1_feature: (B, N, dim)
            modal2_feature: (B, L, dim)
            attn_weight : (B, N, L)
        Returns:
            (B, L, dim)
                Enhancement modal2 multi-modal feature
        """
        F_s = self.max_pooling(modal1_feature)  # [B, D]
        #F_p = self.max_pooling(modal2_feature)  # [B, D]

        #cos_sim = self.cosine_similarity(F_s.unsqueeze(1), modal2_feature)  # [B, L]
        cos_sim = torch.bmm(modal2_feature, F_s.unsqueeze(2)).squeeze(2) # [B, L]
        
        max_cos_sim = self.max_pooling(attn_weight) # [B, L]

        avg_cos_sim = (cos_sim + max_cos_sim) / 2  # [B, L]

        #new_F_p = self.weighted_sum(modal2_feature, avg_cos_sim)  # [B, D]
        new_F_p = avg_cos_sim.unsqueeze(2) * modal2_feature # [B, L, D]

        final_mm_feature = self.alpha * modal2_feature + (1.0 - self.alpha) * new_F_p

        # gate control
        # combined_feature = torch.cat([modal2_feature, new_F_p], dim=2)
        # gate = torch.sigmoid(self.gate_fc(combined_feature))
        # final_mm_feature = gate * modal2_feature + (1.0 - gate) * new_F_p

        #final_mm_feature = self.mlp1(max_words_feature) +  self.mlp2(new_F_p)

        return final_mm_feature

    def pairwise_cosine_similarity(self, x1, x2):
        """
        ...
        return torch.bmm(x1_norm, x2_norm.transpose(1, 2))
    ...
    def max_pooling(self, embeddings):
        # pooling dim=1
        return F.max_pool1d(embeddings.transpose(1, 2), embeddings.size(1)).squeeze(2)

    def weighted_sum(self, emb, weights):
...
class QueryVideoCrossModalEncoderLayer(nn.Module):
    ...
    def forward(self, query_feature, query_mask, video_feature, video_mask):
        ...
        # masked mean
        q2v_attn_m = q2v_attn.masked_fill(query_mask.unsqueeze(2) == 0.0, value=0.0)
        q2v_attn = torch.sum(q2v_attn_m, dim=1) / torch.sum(query_mask, dim=1, keepdim=True)

        v2q, v2q_attn_m = self.v2q(
            query=video_feature,
            key=query_feature,
            value=query_feature,
        ...
        return q2v, v2q, q2v_attn, v2q_attn_m, q2v_attn_m
...
class QueryVideoCrossModalEncoder(nn.Module):
    ...
    def forward(self, query_feature, query_mask, video_feature, video_mask):
        q2v_attn = None
        for layer in self.layers:
            query_feature, video_feature, q2v_attn, v2q_attn_m, q2v_attn_m = layer(
                query_feature, query_mask, video_feature, video_mask
            )
        return query_feature, video_feature, q2v_attn, v2q_attn_m, q2v_attn_m
