...
class GloVe(object):
    """
    Modified GloVe class for more efficient loading of embeddings with caching.
    """
    def __init__(self, glove_path, cache_path='data/glove_cache.pkl'):
    ...
    def contains(self, word):
        return word in self.glove

    def _load(self):
        """ Load GloVe embeddings more efficiently with caching.
        """
        # Check if the cache file exists
        if os.path.isfile(self.cache_path):
            # Load from cache
            print("Glove load from cache")
            with open(self.cache_path, 'rb') as f:
                return pickle.load(f)

        # Load from the original glove file and save to cache
        glove = {}
        with open(self.glove_path, 'r') as f:
            for line in f:
                split_line = line.split()
                word = " ".join(split_line[:-self.dim])  # Handling words with spaces
